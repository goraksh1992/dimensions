...
class Appointment extends CI_Controller {
    ...
    public function create(){
        ...
        if ($check_appointment_exists === false) {
            $message['exception'] = display('your_appointment_already_taken'); 
        ...
        if ($this->form_validation->run() === true && $check_patient_id->status === true && $check_appointment_exists === true) 
        {
            ...
            #-------------------------SMS SEND -----------------------------#
                ...
                /*set success message*/
                $message['message'] = "Appointment scheduled successfully."; 
                $this->session->set_flashdata($message);
    ...
    // appointment create for new patient
    public function new_patient(){
        ...
        $this->form_validation->set_rules('problem', display('problem'),'max_length[255]');
        /* ------------------------------- */
         $data['patient'] = (object)$postPatient = [
        ...
        /* ------------------------------- */
        if ($this->form_validation->run() === true && $check_appointment_exists === true) {

            /*if empty $id then insert data*/
            if ($this->patient_model->create($postPatient)) {
                /*insert appontment data */
                $this->appointment_model->create($postData);
                 ...
                 #accounts information
                   $coa = $this->patient_model->headcode();
                    ...
                    $c_code = $postPatient['patient_id'];
                    $c_name = $postPatient['firstname'].'-'.$postPatient['lastname'];
                    $c_acc=$c_code.'-'.$c_name;
                    $createby = 0;
    ...
    */
    ...
    // for new patient serial
    public function new_patient_serial_by_date()
    {
        $doctor_id  = $this->input->post('doctor_id');
        $date       = date("Y-m-d", strtotime($this->input->post('date'))); 
        $day        = date("l", strtotime($this->input->post('date'))); 
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($result->serial_visibility_type == 2) {
                    ...
                    while ($seq <= $total_serial) {
                        ...
                        if ($this->check_time_sequence($doctor_id, $result->schedule_id, $seq, $date) === true) {
                            //store time sequential
                            $serial .= "<div data-item=\"$seq\" class=\"serial_no slbtn btn btn-primary btn-sm\">$time_from - $time_to</div>"; 
                        } else {
                            /*store time sequential*/
                    ...
                    while ($ts <= $total_serial) {
                        ...
                        if ($this->check_time_sequence($doctor_id, $result->schedule_id, $ts, $date) === true) {
                            //store timestamp
                            $serial .= "<div data-item=\"$ts\" class=\"serial_no slbtn btn btn-primary btn-sm\">".(($ts<=9)?"0$ts":$ts)."</div>";
                        } else {
                            /*store timestamp*/
